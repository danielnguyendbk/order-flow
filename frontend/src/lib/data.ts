...
function buildOrderTimeline(order: Order): OrderTimelineEvent[] {
  ...
  if (order.fulfillmentStatus === "CANCELLED") {
    events.push(ev("CANCELLED", "Hủy đơn", order.expiresAt ?? order.createdAt, "admin", order.adminNote ?? "Khách không thanh toán đúng hạn"));
    if (order.paymentStatus === "REFUNDED") {
      events.push(ev("REFUNDED", `Hoàn tiền ${formatVnd(order.amountVnd)}`, order.expiresAt ?? order.createdAt, "admin", "Hoàn toàn bộ số tiền đã thanh toán"));
    }
    return events;
  }
...
export interface Reconciliation {
  ...
  orderId?: string; // id đơn liên quan (nếu có) — dùng để tạo link /orders/[id]
  sepayId: string;
  amountExpected: number;
  amountReceived: number;
