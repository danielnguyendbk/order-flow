...
export function createTelegramOrderRouter(options: TelegramOrderRouterOptions): Router {
  ...
  router.post("/orders/:orderId/payments/qr/reset", requireServiceStaffOrder, async (req, res, next) => {
    try { res.json(await service.resetQr(getTelegramEmployee(res).id, String(req.params.orderId))); } catch (error) { handleError(error, res, next); }
  });

  router.post("/orders/:orderId/payments/qr/reconcile", requireServiceStaffOrder, async (req, res, next) => {
    try { res.json(await service.reconcileQr(getTelegramEmployee(res).id, String(req.params.orderId))); } catch (error) { handleError(error, res, next); }
  });
