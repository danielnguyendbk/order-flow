...
Implementation status: **implemented**. Public requests return only active
categories and available items. Bot operational requests with
`x-bot-internal-secret` and `x-telegram-user-id` also re-check the employee
